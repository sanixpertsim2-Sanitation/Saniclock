...
<div class="ov" id="actOv"><div class="sheet">
  <h3 id="actName">Employee</h3>
  <div class="msg" id="fpMsg"></div>
  <div style="font-size:12px;color:var(--text2);margin:2px 0 8px">Enroll a fingerprint — pick which finger, then press it on the device.</div>
  <div class="fgrid">
...
/* ---- employee actions / fingerprint enroll ---- */
var actPid=null;
document.addEventListener("click",function(e){
  var row=e.target.closest(".row.tap");if(!row)return;
  actPid=row.getAttribute("data-pid");
  var emp=EMP.filter(function(x){return String(x.pid)===String(actPid);})[0];
  $("#actName").textContent=(emp?emp.person:actPid);
...
Array.prototype.forEach.call(document.querySelectorAll("#actOv .fgrid button"),function(b){b.addEventListener("click",function(){
  if(!actPid)return;var fid=b.getAttribute("data-fid"),msg=$("#fpMsg");
  msg.className="msg";msg.textContent="Sending to device…";
  Array.prototype.forEach.call(document.querySelectorAll("#actOv .fgrid button"),function(x){x.disabled=true;});
  j("/api/ngteco/enroll",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({pid:actPid,fid:+fid})})
    .then(function(r){
      Array.prototype.forEach.call(document.querySelectorAll("#actOv .fgrid button"),function(x){x.disabled=false;});
    ...
    }).catch(function(){Array.prototype.forEach.call(document.querySelectorAll("#actOv .fgrid button"),function(x){x.disabled=false;});msg.className="msg err";msg.textContent="Network error.";});
});});
