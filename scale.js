...
.brand .mark{box-shadow:0 0 0 1px rgba(79,140,255,.16),0 10px 26px -12px var(--glow)!important}
...
<link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;600&display=swap" rel="stylesheet"/>
<style>
:root{--bg:#08090c;--bg2:#0b0d12;--surface:#0f1218;--surface2:#141824;--line:rgba(255,255,255,.08);--line2:rgba(255,255,255,.14);--text:#f3f5f9;--text2:#9aa4b4;--text3:#5b6473;--accent:#4f8cff;--accent2:#7db0ff;--emerald:#34d399;--amber:#fbbf24;--rose:#fb7185;--radius:18px;--glow:rgba(79,140,255,.45);--disp:'Cabinet Grotesk','Satoshi',sans-serif;--mono:'JetBrains Mono',ui-monospace,monospace}
*{box-sizing:border-box;-webkit-tap-highlight-color:transparent;margin:0;padding:0}
html{background:var(--bg)}
body{color:var(--text);font-family:'Satoshi',-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;font-size:15px;overscroll-behavior-y:none;min-height:100vh;background:radial-gradient(900px 520px at 82% -8%,rgba(79,140,255,.12),transparent 58%),radial-gradient(680px 520px at 4% 4%,rgba(143,147,255,.08),transparent 52%),linear-gradient(170deg,var(--bg2),var(--bg) 55%);background-attachment:fixed}
...
button:active,.tab:active,.nav button:active,[data-pg]:active,[data-b]:active{transform:scale(.95)}
.nav button,.tab{transition:transform .12s ease,color .18s ease,background .18s ease}
.page.on{animation:mpage .32s cubic-bezier(.22,1,.32,1)}
@keyframes mpage{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}
.card{animation:mpage .4s cubic-bezier(.22,1,.32,1) both}
h1,h2,h3{font-family:var(--disp);font-weight:800;letter-spacing:-.02em}
</style></head>
<body>
<div class="top">
...
<style>
:root{
  --bg:#08090c; --bg2:#0b0d12; --surface:#0f1218; --surface2:#141824;
  --line:rgba(255,255,255,.07); --line2:rgba(255,255,255,.13);
  --ink:#f3f5f9; --ink2:#9aa4b4; --ink3:#5b6473;
  --disp:'Cabinet Grotesk','Satoshi',sans-serif;
  --sans:'Satoshi',ui-sans-serif,system-ui,-apple-system,sans-serif;
  --mono:'JetBrains Mono',ui-monospace,Menlo,monospace;
  --brand:#4f8cff; --brand2:#7db0ff; --brand3:#a9ccff; --brand-hover:#3f7bf0; --glow:rgba(79,140,255,.5);
  --day:#f5a524; --aft:#fb7a54; --night:#8f93ff; --emerald:#34d399;
}
:root{--ink3:#5b6473}
...
/* LEFT: form */
.left{flex:0 0 42%;max-width:520px;position:relative;background:var(--surface);border-right:1px solid var(--line);display:flex;flex-direction:column;padding:52px 58px;overflow:hidden}
.left::before{content:"";position:absolute;top:-30%;left:-20%;width:420px;height:420px;background:radial-gradient(circle,var(--glow),transparent 62%);opacity:.14;pointer-events:none}
@media (max-width:960px){.left{flex:none;max-width:none;border-right:0;border-bottom:1px solid var(--line);padding:36px 26px}}
.left>*{position:relative;z-index:2}
.brandbar{display:flex;align-items:center;gap:14px;opacity:0;animation:up .7s .04s cubic-bezier(.22,1,.32,1) forwards}
.brandbar img{width:48px;height:48px;border-radius:12px;border:1px solid var(--line2);object-fit:contain;background:#0a0c11}
.brandbar b{display:block;font-family:var(--disp);font-size:22px;font-weight:800;letter-spacing:-.02em;line-height:1}
.brandbar span{display:block;margin-top:4px;font-size:10px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:var(--brand2)}
.formwrap{margin:auto 0;max-width:370px;width:100%}
.eyebrow{font-size:11px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:var(--ink3);margin-bottom:14px;opacity:0;animation:up .7s .12s cubic-bezier(.22,1,.32,1) forwards}
h1{font-family:var(--disp);font-size:46px;font-weight:800;letter-spacing:-.03em;line-height:.98;margin-bottom:12px;opacity:0;animation:up .7s .16s cubic-bezier(.22,1,.32,1) forwards}
.lead{font-size:15px;color:var(--ink2);line-height:1.55;margin-bottom:32px;opacity:0;animation:up .7s .22s cubic-bezier(.22,1,.32,1) forwards}
.field{margin-bottom:16px;opacity:0;animation:up .7s cubic-bezier(.22,1,.32,1) forwards}
.field:nth-of-type(1){animation-delay:.28s}.field:nth-of-type(2){animation-delay:.34s}
.field label{display:block;font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--ink2);margin-bottom:8px}
.inp{position:relative}
.inp svg{position:absolute;left:15px;top:50%;transform:translateY(-50%);width:17px;height:17px;color:var(--ink3);transition:color .2s}
.inp:focus-within svg{color:var(--brand2)}
.inp input{width:100%;height:52px;background:#0a0c11;border:1px solid var(--line);border-radius:11px;color:var(--ink);font-family:var(--sans);font-size:15px;padding:0 14px 0 44px;outline:none;transition:border-color .2s,box-shadow .2s,background .2s}
.inp input::placeholder{color:#4b5567}
.inp input:focus{border-color:var(--brand);background:#0c0f16;box-shadow:0 0 0 4px rgba(79,140,255,.16)}
.btn{position:relative;overflow:hidden;width:100%;height:52px;margin-top:22px;border:0;border-radius:11px;cursor:pointer;font-family:var(--sans);font-size:15px;font-weight:700;color:#fff;background:linear-gradient(135deg,var(--brand2),var(--brand));box-shadow:0 14px 34px -14px var(--glow),inset 0 1px 0 rgba(255,255,255,.25);transition:transform .12s ease,box-shadow .2s ease,filter .2s;display:flex;align-items:center;justify-content:center;gap:9px;opacity:0;animation:up .7s .4s cubic-bezier(.22,1,.32,1) forwards}
.btn:hover{transform:translateY(-2px);box-shadow:0 20px 44px -14px var(--glow)}
.btn:active{transform:translateY(0) scale(.985)}
.btn[disabled]{opacity:.65;cursor:default}
.ripple{position:absolute;border-radius:50%;transform:translate(-50%,-50%) scale(0);background:rgba(255,255,255,.4);pointer-events:none;animation:rip .55s ease-out forwards}
@keyframes rip{to{transform:translate(-50%,-50%) scale(1);opacity:0}}
.err{background:rgba(248,113,113,.1);border:1px solid rgba(248,113,113,.3);color:#fca5a5;font-size:13px;padding:11px 13px;border-radius:10px;margin-bottom:16px;display:none}
.trust{margin-top:26px;display:flex;align-items:center;gap:8px;font-size:12px;color:var(--ink3);opacity:0;animation:up .7s .5s cubic-bezier(.22,1,.32,1) forwards}
.trust svg{width:14px;height:14px}
.leftfoot{margin-top:auto;padding-top:30px;font-size:11.5px;color:var(--ink3);opacity:0;animation:up .7s .6s cubic-bezier(.22,1,.32,1) forwards}
.spin{width:17px;height:17px;border:2px solid rgba(255,255,255,.45);border-top-color:#fff;border-radius:50%;animation:sp .6s linear infinite}
@keyframes sp{to{transform:rotate(360deg)}}
@keyframes up{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:none}}

/* RIGHT: activity + clock */
.right{flex:1;min-width:0;position:relative;display:flex;flex-direction:column;justify-content:center;gap:26px;padding:52px 60px;overflow:hidden;background:radial-gradient(1000px 560px at 78% -12%,rgba(79,140,255,.12),transparent 60%),radial-gradient(720px 520px at 8% 108%,rgba(143,147,255,.08),transparent 58%),var(--bg)}
.right::before{content:"";position:absolute;inset:0;pointer-events:none;background-image:linear-gradient(rgba(255,255,255,.03) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.03) 1px,transparent 1px);background-size:58px 58px;mask-image:radial-gradient(ellipse 82% 76% at 46% 46%,#000,transparent 92%);-webkit-mask-image:radial-gradient(ellipse 82% 76% at 46% 46%,#000,transparent 92%)}
@media (max-width:960px){.right{padding:32px 24px 44px;gap:20px}}
.right>*{position:relative;z-index:2}
.clockrow{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;flex-wrap:wrap}
.clockbox{opacity:0;animation:up .7s .1s cubic-bezier(.22,1,.32,1) forwards}
.clocklabel{font-size:11px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:var(--ink3);display:flex;align-items:center;gap:10px;margin-bottom:12px}
.clocklabel .live{width:7px;height:7px;border-radius:50%;background:var(--emerald);position:relative}
.clocklabel .live::after{content:"";position:absolute;inset:-4px;border-radius:50%;border:1px solid var(--emerald);opacity:.5;animation:ping 2s ease-out infinite}
@keyframes ping{0%{transform:scale(.6);opacity:.6}100%{transform:scale(1.9);opacity:0}}
.clock{font-family:var(--disp);font-size:clamp(64px,8.5vw,120px);font-weight:800;letter-spacing:-.05em;line-height:.86;color:var(--ink);font-variant-numeric:tabular-nums}
.clock .sec{font-family:var(--mono);font-weight:500;color:var(--brand2);font-size:.26em;letter-spacing:0;vertical-align:.9em;margin-left:.12em}
.date{margin-top:12px;font-size:14.5px;color:var(--ink2);font-weight:500}
.statchips{display:flex;gap:10px;flex-wrap:wrap;opacity:0;animation:up .7s .2s cubic-bezier(.22,1,.32,1) forwards}
.chip{display:flex;align-items:center;gap:9px;background:var(--surface);border:1px solid var(--line);border-radius:11px;padding:11px 14px}
.chip .ci{width:30px;height:30px;flex:none;border-radius:8px;display:grid;place-items:center;background:var(--surface2);color:var(--brand2)}
.chip .ci svg{width:16px;height:16px}
.chip b{font-family:var(--disp);font-size:15px;font-weight:800;letter-spacing:-.01em;display:block;line-height:1}
.chip span{font-size:10.5px;color:var(--ink3);font-weight:600}

/* activity graph card */
.graph{background:linear-gradient(180deg,var(--surface),#0c0f15);border:1px solid var(--line);border-radius:18px;padding:20px 22px 14px;box-shadow:0 30px 70px -34px rgba(0,0,0,.8);opacity:0;transform:translateY(16px);animation:up .8s .26s cubic-bezier(.22,1,.32,1) forwards}
.graph .gh{display:flex;align-items:center;justify-content:space-between;margin-bottom:4px}
.graph .gt b{font-family:var(--disp);font-size:15px;font-weight:800;letter-spacing:-.01em}
.graph .gt span{display:block;font-size:11.5px;color:var(--ink3);margin-top:2px}
.graph .gpill{font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--brand2);background:rgba(79,140,255,.12);border:1px solid rgba(79,140,255,.26);padding:5px 11px;border-radius:999px}
.chartwrap{position:relative;margin-top:8px}
.chartwrap svg{display:block;width:100%;height:auto}
.gline{fill:none;stroke:url(#gstroke);stroke-width:2.5;stroke-linecap:round;stroke-linejoin:round;stroke-dasharray:1;stroke-dashoffset:1;animation:draw 1.9s .5s cubic-bezier(.4,0,.2,1) forwards}
@keyframes draw{to{stroke-dashoffset:0}}
.garea{fill:url(#garea);opacity:0;animation:fadein 1.2s 1.1s ease forwards}
@keyframes fadein{to{opacity:1}}
.gdot{fill:var(--brand2);animation:dotp 2s 1.6s ease-in-out infinite}
.gdot-h{fill:none;stroke:var(--brand2);stroke-width:1.5;opacity:.5;transform-origin:center;animation:dotring 2s 1.6s ease-out infinite}
@keyframes dotp{0%,100%{r:4}50%{r:5.4}}
@keyframes dotring{0%{r:4;opacity:.6}100%{r:14;opacity:0}}
.gaxis{display:flex;justify-content:space-between;margin-top:6px;padding:0 2px}
.gaxis span{font-family:var(--mono);font-size:10px;color:var(--ink3)}
.gscan{stroke:rgba(255,255,255,.14);stroke-width:1;animation:scan 6s 1.4s linear infinite}
@keyframes scan{0%{transform:translateX(0);opacity:0}8%{opacity:1}92%{opacity:1}100%{transform:translateX(600px);opacity:0}}

/* shift strip */
.shifts{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;opacity:0;animation:up .7s .34s cubic-bezier(.22,1,.32,1) forwards}
@media (max-width:520px){.shifts{grid-template-columns:1fr}}
.shift{position:relative;overflow:hidden;background:var(--surface);border:1px solid var(--line);border-radius:13px;padding:13px 14px;transition:border-color .35s,transform .35s,box-shadow .35s}
.shift .st{display:flex;align-items:center;justify-content:space-between}
.shift .sn{font-family:var(--disp);font-size:15px;font-weight:800;color:var(--ink3);transition:color .3s}
.shift .sb{font-size:8.5px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:#04070d;background:var(--sc);padding:3px 7px;border-radius:999px;opacity:0;transition:opacity .3s}
.shift .sr{margin-top:5px;font-family:var(--mono);font-size:11.5px;color:var(--ink3);transition:color .3s}
.shift .sbar{position:relative;margin-top:11px;height:4px;border-radius:999px;background:rgba(255,255,255,.08);overflow:hidden}
.shift .sf{position:absolute;left:0;top:0;bottom:0;width:0;border-radius:999px;background:var(--sc);transition:width 1s cubic-bezier(.22,1,.32,1)}
.shift .sm{margin-top:8px;font-size:10px;color:var(--ink3);font-weight:600;font-variant-numeric:tabular-nums}
.shift.now{border-color:var(--sc);transform:translateY(-3px);box-shadow:0 16px 40px -20px var(--sc)}
.shift.now .sn,.shift.now .sr{color:var(--ink)}
.shift.now .sb{opacity:1}
.shift.now .sm{color:var(--brand2)}
.shift[data-s=day]{--sc:var(--day)}.shift[data-s=aft]{--sc:var(--aft)}.shift[data-s=night]{--sc:var(--night)}
.rightfoot{display:flex;justify-content:space-between;gap:12px;font-size:11.5px;color:var(--ink3);opacity:0;animation:up .7s .6s cubic-bezier(.22,1,.32,1) forwards}
.rightfoot b{color:var(--ink2);font-weight:700}
@media (prefers-reduced-motion:reduce){*{animation:none!important}[class]{opacity:1!important;transform:none!important}.gline{stroke-dashoffset:0}}
</style></head>
<body>
<section class="left">
  <div class="brandbar"><img src="/brand-badge.png" alt="SaniXperts"/><div><b>SaniClock</b><span>Powered by SaniXperts</span></div></div>
  <div class="formwrap">
    <div class="eyebrow">Workspace access</div>
    <h1>Welcome back.</h1>
    <div class="lead">Sign in to your attendance &amp; payroll command center.</div>
    <div class="err" id="err" data-testid="login-error"></div>
    <form id="f" data-testid="login-form">
      <div class="field"><label for="u">Username</label><div class="inp"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><input id="u" name="u" autocomplete="username" placeholder="username" autofocus data-testid="login-username-input"/></div></div>
      <div class="field"><label for="p">Password</label><div class="inp"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg><input id="p" name="p" type="password" autocomplete="current-password" placeholder="••••••••" data-testid="login-password-input"/></div></div>
      <button class="btn" id="btn" type="submit" data-testid="login-submit-btn"><span id="btxt">Sign in</span></button>
    </form>
    <div class="trust"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg>256-bit encrypted session · SaniXperts</div>
  </div>
  <div class="leftfoot">© 2026 SaniXperts · SaniClock Attendance &amp; Payroll</div>
</section>

<section class="right">
  <div class="clockrow">
    <div class="clockbox">
      <div class="clocklabel"><span class="live"></span>Live facility time</div>
      <div class="clock" id="clock" data-testid="live-clock">--:--<span class="sec" id="sec">--</span></div>
      <div class="date" id="date">—</div>
    </div>
    <div class="statchips">
      <div class="chip"><span class="ci"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v4M12 2a10 10 0 1 0 10 10"/></svg></span><div><b>24 / 7</b><span>Coverage</span></div></div>
      <div class="chip"><span class="ci"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12h4l3 8 4-16 3 8h4"/></svg></span><div><b>3 shifts</b><span>Rotating</span></div></div>
      <div class="chip"><span class="ci"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M3 10h18M8 2v4M16 2v4"/></svg></span><div><b>Auto</b><span>Payroll</span></div></div>
    </div>
  </div>

  <div class="graph">
    <div class="gh"><div class="gt"><b>Attendance rhythm</b><span>Clock-in volume across a typical 24 hours</span></div><span class="gpill">Live sync</span></div>
    <div class="chartwrap">
      <svg viewBox="0 0 600 210" preserveAspectRatio="none" role="img" aria-label="Attendance activity chart">
        <defs>
          <linearGradient id="gstroke" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="#8f93ff"/><stop offset="0.5" stop-color="#4f8cff"/><stop offset="1" stop-color="#7db0ff"/></linearGradient>
          <linearGradient id="garea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#4f8cff" stop-opacity="0.32"/><stop offset="1" stop-color="#4f8cff" stop-opacity="0"/></linearGradient>
        </defs>
        <line x1="0" y1="52" x2="600" y2="52" stroke="rgba(255,255,255,.05)"/>
        <line x1="0" y1="105" x2="600" y2="105" stroke="rgba(255,255,255,.05)"/>
        <line x1="0" y1="158" x2="600" y2="158" stroke="rgba(255,255,255,.05)"/>
        <path class="garea" d="M0,150 C40,150 70,58 150,68 C210,76 232,150 262,144 C300,138 322,64 362,68 C410,73 432,150 472,150 C520,150 542,54 600,62 L600,210 L0,210 Z"/>
        <path class="gline" pathLength="1" d="M0,150 C40,150 70,58 150,68 C210,76 232,150 262,144 C300,138 322,64 362,68 C410,73 432,150 472,150 C520,150 542,54 600,62"/>
        <line class="gscan" x1="0" y1="0" x2="0" y2="210"/>
        <circle class="gdot-h" cx="542" cy="56" r="4"/>
        <circle class="gdot" cx="542" cy="56" r="4"/>
      </svg>
    </div>
    <div class="gaxis"><span>00</span><span>04</span><span>08</span><span>12</span><span>16</span><span>20</span><span>24</span></div>
  </div>

  <div class="shifts" id="shifts">
    <div class="shift" data-s="day"><div class="st"><span class="sn">Day</span><span class="sb">On now</span></div><div class="sr">07:00 – 15:00</div><div class="sbar"><span class="sf"></span></div><div class="sm" data-meta>8-hour shift</div></div>
    <div class="shift" data-s="aft"><div class="st"><span class="sn">Afternoon</span><span class="sb">On now</span></div><div class="sr">15:00 – 23:00</div><div class="sbar"><span class="sf"></span></div><div class="sm" data-meta>8-hour shift</div></div>
    <div class="shift" data-s="night"><div class="st"><span class="sn">Night</span><span class="sb">On now</span></div><div class="sr">23:00 – 07:00</div><div class="sbar"><span class="sf"></span></div><div class="sm" data-meta>8-hour shift</div></div>
  </div>
  <div class="rightfoot"><span>Every shift, accounted for.</span><span><b>Ferrero</b> · Attendance &amp; Payroll</span></div>
</section>
...
function tick(){
  ...
  Array.prototype.forEach.call(document.querySelectorAll(".shift"),function(c){
    ...
    var fill=c.querySelector(".sf"),meta=c.querySelector("[data-meta]"),bdg=c.querySelector(".sb");if(bdg)bdg.setAttribute("aria-hidden",isNow?"false":"true");if(!fill||!meta)return;
    if(isNow){var cm=(k==="night"&&mins<420)?mins+1440:mins;var span=SH[k].e-SH[k].s,el=cm-SH[k].s,left=span-el;
...
document.addEventListener("pointerdown",function(e){var b=e.target.closest(".btn");if(!b)return;var r=b.getBoundingClientRect();var sp=document.createElement("span");sp.className="ripple";var sz=Math.max(r.width,r.height)*1.2;sp.style.width=sp.style.height=sz+"px";sp.style.left=(e.clientX-r.left)+"px";sp.style.top=(e.clientY-r.top)+"px";b.appendChild(sp);setTimeout(function(){sp.remove();},560);});
