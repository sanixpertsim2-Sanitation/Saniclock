...
const PORT = process.env.FERRERO_PORT || 8400;
const PASS = process.env.FERRERO_PASS || 'Ferrero2026';
const SECRET = process.env.FERRERO_SECRET || crypto.randomBytes(16).toString('hex');
const API = 'office-api.ngteco.com';
const TC7_ID = 'ee8959801e404554af7b2e8ba5623518';       // NG-TC7, Ferrero, SN CCH5252300480
const FERRERO_DEPT = '8a8294229fcaca2d01a01058c1db030e'; // department "Ferrero" (code 003)
const DESIG = '8a8294659f3fbee2019f6727c86305a9';        // DEFAULT designation
...
async function ferreroRoster(tok) {
  ...
  while (true) {
    ...
    for (const e of rows) {
      ...
      let appAccess = false, appEmail = '';
      if (empAuth) { try { const a = empAuth.byPid(String(code)); if (a) { appAccess = true; appEmail = a.email || ''; } } catch (x) {} }
...
async function enrollCredential(tok, type, code, personId, fid) {
  ...
  if (type === 'fingerprint') body.fid = String(fid == null ? 6 : fid); // 6 = right index default
...
async function addEmployee(tok, f) {
  const body = { firstName: f.firstName, lastName: f.lastName || '.', code: String(f.code), email: f.email || '', departmentIdOrCode: FERRERO_DEPT, designationIdOrCode: DESIG, createUser: false };
  ...
  try {
    ...
    if (url.startsWith('/api/')) {
      ...
      if (url === '/api/adduser' && req.method === 'POST') {
        ...
        return json(res, 200, await addEmployee(tok, b));
      }
...
<title>Ferrero Enrollment</title>
<style>
...
<div class="ov" id="fingerOv"><div class="sheet">
  ...
  <p id="fingerWho">Pick a finger, then the employee presses it on the TC7 <b>3 times</b>.</p>
...
<div class="ov" id="punchOv"><div class="sheet">
  ...
  <p id="punchWho">Ask the employee to <b>punch once</b> on the TC7 now to confirm their fingerprint works. Click below once they&#39;ve punched.</p>
...
<div class="ov" id="addOv"><div class="sheet">
  <h3>Add new Ferrero employee</h3>
  <p>Creates the person in NGTeco (Ferrero department) live. They&#39;ll then appear in Pending, ready to enroll.</p>
...
function render(j){
  ...
  var pills='<span class="pill '+(d.status==='online'?'on':'off')+'">'+(d.status==='online'?'TC7 online':'TC7 offline')+'</span>'+
...
function paint(){
  ...
  list.innerHTML=rows.map(function(e){
    ...
    return '<div class="row"><span class="av" style="'+avatar(e.name)+'">'+esc(ini(e.name))+'</span>'+
      '<div class="info"><b>'+esc(e.name)+'</b><span>ID '+esc(e.code)+(e.email?' \\u00b7 '+esc(e.email):'')+'</span></div>'+acts+'</div>';
  }).join('');
}
/* tabs + search */
...
function openFingerPicker(name){
  $('#fingerWho').innerHTML='For <b>'+esc(name)+'</b>: pick a finger, tap Start, then they press it on the TC7 <b>3 times</b>.';
  $('#fingerGrid').innerHTML=FINGERS.map(function(f,i){return '<button data-fid="'+f[1]+'"'+(f[1]===6?' class="sel"':'')+'>'+f[0]+'</button>';}).join('');
  STATE.pending.fid=6;
...
function fireEnroll(type,code,id,fid,name){
  toast('Sending to TC7\\u2026');
  jpost('/api/enroll',{type:type,code:code,personId:id,fid:fid}).then(function(r){
    if(r.ok){
      toast('\\u2713 '+type.charAt(0).toUpperCase()+type.slice(1)+' armed on the TC7 for '+name+'. NGTeco: '+(r.message||'Success'),'ok');
...
function openPunchVerify(name,code){
  $('#punchWho').innerHTML='Ask <b>'+esc(name)+'</b> to press their finger to <b>punch once</b> on the TC7 now. Then click below to confirm it registered.';
...
$('#addSubmit').addEventListener('click',function(){
  ...
  var payload={firstName:$('#afn').value.trim(),lastName:$('#aln').value.trim(),code:$('#acode').value.trim(),email:$('#aemail').value.trim()};
  jpost('/api/adduser',payload).then(function(r){
    b.disabled=false;
    if(r.ok){$('#addOv').classList.remove('on');toast('\\u2713 '+payload.firstName+' created in NGTeco. Now in Pending.','ok');STATE.tab='pending';Array.prototype.forEach.call(document.querySelectorAll('.tab'),function(x){x.classList.toggle('on',x.getAttribute('data-tab')==='pending');});load();}
